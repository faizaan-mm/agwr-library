...
from .utils import local_dist, kernel_funcs, alt, calculate_dependent, R2
...
class SMGWRModel:
    ...
    def __init__(self, X_training, y_training, coords_training, config={}):
        ...
        self.config = {"cutoff": -1, "kernel_function": 'gaussian', "eps": 1.0000001,
                       "spherical": True, "fixed": False, "validation": "normal"}
    ...
    def fit(self, bw, indices=[], iterations=3):
        ...
        convergence_it = 0
        old_pred = 10
        new_pred = R2(self.y_training[indices],
                            calculate_dependent(self.trainB[indices, :], self.X_training[indices, :]))
        ...
        while convergence_it < iterations and (new_pred / old_pred) < 0.999:
            ...
            new_pred = R2(self.y_training[indices], self.y_training[indices] + self.y_residuals[indices])
