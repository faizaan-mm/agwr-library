...
from .DataDivider import Divider
...
class ModularFramework:
    ...
    def __init__(self, spatial_model=None, fast_model=None, config={}):
        ...
        if "divide_method" not in config:
            config["divide_method"] = "random"

        self.process_count = config["process_count"] if config["process_count"] else 1
    ...
    def _get_predict_array(self, prediction):
        """
        Safely retrieves the prediction array from the model output.
        """
        if hasattr(prediction, 'predy'):
            return getattr(prediction, 'predy')
        else:
            return prediction
